document.addEventListener('alpine:init', () => {
    Alpine.data('checkout', () => ({
        ...
        fetchCourier() {
            if (this.destination !== '') {
                fetch(`https://ongkir-${this.user.origin_id}.netlify.app/api/${this.destination.split(',')[0]}`)
        ...
        sendWa(products, subtotal, berat) {
            ...
            const hp = `*Nomor%20HP*%20%3A%20${(this.hp).toString().replace(/\D/g, '').replace(new RegExp('^0'), '62').replace(new RegExp('^8'), '628')}%20%0A`;
