document.addEventListener('alpine:init', () => {
    Alpine.data('checkout', () => ({
        ...
        sendWa(products, subtotal, berat) {
            ...
            const waktu = `*Waktu*%20%20%3A%20${this.getTime()}%0A%0A`;
            ...
            const kurir = `*Kurir*%20%20%20%20%20%20%20%3A%20${this.courier !== '' ? this.courier.split(',')[0] + ' - ' + this.localPrice(parseInt(this.courier.split(',')[1])) : '' }%0A`;
            ...
            const link = `https://api.whatsapp.com/send?phone=${this.user.whatsapp}&text=${header}${name}${hp}${email}${address}${pembayaran}${kurir}${catatan}${product_list}${sub_total}${ongkir}${total_bayar}`;
