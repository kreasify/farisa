document.addEventListener('alpine:init', () => {
    Alpine.data('checkout', () => ({
        ...
        sendWa(products, subtotal, berat) {
            ...
            const kurir = `*Kurir*%20%20%20%20%20%20%20%3A%20${this.courier !== '' ? (this.courier.split(',')[0]).replace(new RegExp(escapeRegex(i), 'g'), map[i]) + ' - ' + this.localPrice(parseInt(this.courier.split(',')[1])) : '' }%0A`;
